...
window.BITMEX_NONCE = 0
window.BITMEX_NONCE_OFFSET = -1
...
function BitMEX() {
	...
	function* getNonce() {
		...
		if (window.BITMEX_NONCE != nonce) {
			window.BITMEX_NONCE_OFFSET = -1
		}

		window.BITMEX_NONCE = nonce
		window.BITMEX_NONCE_OFFSET++

		nonce += window.BITMEX_NONCE_OFFSET
	...
	function* positionsClose(Command, position) {
		...
		if (Command.u === "currency" && !Command.q.getIsPercent()) {
			...
			Command.q = new NumberObject(contracts)
		}
		...
		if (Command.sl) {
			params.ordType = "Stop"
			params.stopPx = Command.sl.relative(price).resolve(market.precision)
		}
		if (Command.tp) {
			params.ordType = "MarketIfTouched"
			params.stopPx = Command.tp.relative(price).resolve(market.precision)
		}
