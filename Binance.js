...
function Binance() {
	...
	function* trade(Command) {
		...
		params.quantity = Command.q.resolve(quantityPrecision)
		params.newOrderRespType = "ACK" // ACK, RESULT, FULL

		if (Command.sl) {
			params.type = Command.t === "market" ? "STOP_LOSS" : "STOP_LOSS_LIMIT"
			params.stopPrice = Command.sl.relative(price).resolve(pricePrecision)
		}
		if (Command.tp) {
			params.type = Command.t === "market" ? "TAKE_PROFIT" : "TAKE_PROFIT_LIMIT"
			params.stopPrice = Command.tp.relative(price).resolve(pricePrecision)
		}

		let resource = "/v3/order"
		if (Command.d) {
			console.info(this.getExchangeName(), params)
			return false // Disabled
		}

		return yield* post.call(this, resource, params, SECURITY_TRADE)
	}
