...
function Kraken() {
	...
	function* trade(Command) {
		...
		if (Command.sl && Command.sl.getIsPercent()) {
			Command.sl.relative(price)
		}
		if (Command.sl && Command.tp.getIsPercent()) {
			Command.tp.relative(price)
		}
		...
		if (Command.hasOwnProperty("sl") && Command.hasOwnProperty("tp")) {
			...
			params.price = Command.sl.resolve(market.pair_decimals) // stop loss price
			params.price2 = Command.tp.resolve(market.pair_decimals) // take profit price
			...
			if (Command.t === "market") {
				...
				params.price = Command.sl.resolve(market.pair_decimals) // stop loss trigger price
				params.price2 = price // triggered limit price
			}
		} else if (Command.hasOwnProperty("tp")) {
			if (Command.t === "market") {
				params.ordertype = "take-profit"
				params.price = Command.tp.resolve(market.pair_decimals) // take profit price
			} else {
				params.ordertype = "take-profit-limit"
				params.price = Command.tp.resolve(market.pair_decimals) // take profit price
				params.price2 = price // triggered limit price
			}
			params.price = Command.tp.resolve(market.pair_decimals)
		} else if (Command.hasOwnProperty("ts")) {
			if (Command.t === "market") {
				params.ordertype = "trailing-stop"
				params.price = Command.ts.resolve(market.pair_decimals) // trailing stop offset
			} else {
